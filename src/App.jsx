...
export default function App() {
  ...
  useEffect(() => {
    const snapshot = { dispensing, doorFlow, doorClosed, pickupSecondsLeft, orderData };
  ...
  useEffect(() => {
    ...
    if (pickupSecondsLeft <= 0 && !doorClosed) setDoorFlow(DOOR_STATES.WAIT_CLOSE);
  ...
  switch (screen) {
    case SCREENS.CATEGORIES:
      return <><Header {...headerProps} /><CategoriesScreen lang={lang} categories={categories} loading={loading} onSelect={(cat) => { setCategory(cat); setScreen(SCREENS.CATALOG); resetIdleTimer(); }} /></>;
    case SCREENS.CATALOG:
      return <><Header {...headerProps} /><CatalogScreen lang={lang} inventory={inventory.filter(i => i.product_category === category)} category={category} cart={cart} onAdd={addToCart} onRemove={removeFromCart} onBack={() => setScreen(SCREENS.CATEGORIES)} /></>;
    case SCREENS.CART:
      return <><Header {...headerProps} /><CartScreen lang={lang} cart={cart} total={cartTotal} onAdd={addToCart} onRemove={removeFromCart} onClear={() => setCart([])} onBack={() => setScreen(SCREENS.CATALOG)} onCheckout={() => setScreen(SCREENS.PAYMENT)} /></>;
    case SCREENS.PAYMENT:
      return <><Header {...headerProps} /><PaymentScreen lang={lang} cart={cart} total={cartTotal} onSuccess={handlePaymentSuccess} onError={handleError} onCancel={() => setScreen(SCREENS.CART)} /></>;
    case SCREENS.ERROR:
      return <><Header {...headerProps} /><ErrorScreen lang={lang} message={error} onRetry={handleReset} /></>;
    default:
      return <IdleScreen lang={lang} onSelectLang={handleSelectLang} />;
  }
}
